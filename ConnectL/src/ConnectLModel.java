...
public class ConnectLModel extends JApplet implements MouseListener {
	...
	private class Canvas extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			for (int r=0; r< 3; r++) {
				for ( int c = 0; c< 3; c++) {
					if ( game.getMarkIn(r, c) == tttGame.MARK_O) {
						g.setColor(Color.WHITE);
						g.fillOval(c*w/3, r*h/3, w/3, h/3);
					}
					if ( game.getMarkIn(r, c) == tttGame.MARK_X) {
	...
	private class uiHandler implements ActionListener { // The event
		// listener.
...
void updateStatus() {
	String status;
	if ( game.getGameState() == tttGame.GAME_STATE_O_TURN)
		status = " O's Turn";
